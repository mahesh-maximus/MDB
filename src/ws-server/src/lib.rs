...
lazy_static! {
    ...
    static ref SEND_QUEUE: Mutex<VecDeque<String>> = {
                ...
                spawn(move || {
                    ...
                    loop {
                        ...
                        while SEND_QUEUE.lock().unwrap().len() > 0 {
                            let message = SEND_QUEUE.lock().unwrap().pop_front().unwrap().to_string();

                            println!("WebSocketServer.bind_and_run, try to send message: '{}' to client", message);
...
mod tests {
    ...
    fn xyz() {
        ...
        ws.write_message("Message from WS client".to_string());

        println!("_________________ : {}", ws.get_send_queue_len().to_string());
